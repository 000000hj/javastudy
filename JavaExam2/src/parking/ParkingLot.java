...
public class ParkingLot {
  ...
  public void addCar(){
    ...
    name=sc.next();
    cars.add(new Car(no,name));
    System.out.println("차량번호 "+no+" 차량이 등록되었습니다");
  ...
  public void deleteCar()throws RuntimeException{
    ...
    for(Car car : cars) {
      if(no.equals(car.getCarNo())) {
        cars.remove(car);
        System.out.println("차량번호" +no+ "차량이 삭제되었습니다.");
        return;
      }
     
    
    }
    System.out.println("차량번호"+no+"차량이 존재하지 않습니다.");
  }
  ...
  public void printAllCars() throws RuntimeException{
    ...
    for(int i=0 , length=MAX-size;i<length;i++)
    {
      System.out.println((i+1+length)+"번째 차량: 없음");
    }
