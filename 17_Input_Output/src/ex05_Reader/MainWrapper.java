...
public class MainWrapper {
  ...
  public static void ex01()
  {
    ...
    FileReader fr=null;
    try {
      //파일 입력스트림 생성(반드시 예외처리 필요, 파일이 없으면 FileNotFoundException 발생)
      fr=new FileReader(file);
      
      //입력된 문자 저장 변수
      int ch=0;
      //입력된 문자를 누적할 StringBuffer생성
      StringBuffer sb=new StringBuffer();
      
      //read()메소드
      //1. 1개 문자를 읽어서 반환한다.
      //2. 읽은 문자가 없으면 -1을 반환한다.
      //반복문 : 읽은 문자가 -1 이 아니면 계속 읽는다.
  ...
  public static void ex03()
  {
    ...
    try {
      //버퍼 입력스트림 생성(반드시 예외처리 필요, 파일이 없으면 FileNotFoundException 발생)
      ...
      //반복문 : readLine()의 반환값이 null 이 아니면 계속 읽는다.
      while((line=br.readLine())!=null)
      {
        sb.append(line+"\n");//읽은 라인에 줄 바꿈은 포함되어 있지 않다.
      }
  ...
  public static void ex04()
  {
     ...
     * 1.  Reader클래스를 상속 받는 클래스이다.(문자 입력스트림이다)
     * 2.  InputStream(바이트 입력스트림)을 받아서  Reader(문자입력 스트림)으로 변환한다.
     */
