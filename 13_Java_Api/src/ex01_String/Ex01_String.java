...
public class Ex01_String {
  ...
  {
   //toUpperCase메소드:모두 대문자로 변환
   //toLowerCase메소드:모두 소문자로 변환
  ...
  public static void ex07()
  {
    ...
    String lastName=name.substring(0);
  ...
  public static void ex08()
  {
    //indexOf메소드
    //1.특정 문자열의 인덱스를 반환
    //2.특정 문자열이 2개 이상 존재하는경우 발견된 첫 번째 문자열의 인덱스를 반환
    //3.발견되지 않으면 -1을 반환
...
//contains메소드
    ...
    if(email.contains("@")) {
      System.out.println("@가 포함되어있다.");
      
    }
    else {
      System.out.println("@가 포함되어있지 않다");
    }
  ...
  public static void ex11() {
    ...
    //isBlank메소드
    //1.빈 문자열이거나, 공백문자로 구성되었다면 true 반환 아니면 false 반환
 ...
 public static void ex12() { 
   ...
   //출력 폭 조정
   System.out.println(String.format("%5d",number));//5자리 10진수로 표시 ,숫자를 오른쪽 정렬처리
   System.out.println(String.format("%-5d",number));//5자리 10진수로 표시 ,숫자를 왼쪽 정렬처리
   System.out.println(String.format("%05d",number));//5자리 10진수로 표시 ,숫자를 오른쪽 정렬처리,빈자리를 0으로 채움
   
   System.out.println(String.format("%10s", str));//10자리 문자열표시, 문자열을 오른쪽 정렬처리
   System.out.println(String.format("%-10s", str));//10자리 문자열표시, 문자열을 왼쪽 정렬처리
 }
 ...
 public static void ex13() {
   ...
   System.out.println(str);//192.168.0.101을 기대하지만 정규식에서 .는 모든 문자를 의미하므로 원하는 결과를 얻을 수 없다.
 }
...
{
  // replace 메소드
  // 1.지정된 문자열을 다른 문자열로 변환하는 역할을 수행
  // 2. 발견된 모든 문자열을 변환
  String ip="192.168.0.101";
